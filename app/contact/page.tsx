...
export default function ContactBookPage() {
    ...
    const [students, setStudents] = useState<any[]>([]); // 學生名單
    const [myBooks, setMyBooks] = useState<any[]>([]);   // 聯絡簿列表 (家長用) or 歷史紀錄 (老師用)
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (error) {
            ...
            // alert('聯絡簿已發送！📝'); // 拿掉 alert，直接更新畫面比較順暢
            setForm({ content: '', homework: '' }); // 清空表單
            fetchHistory(selectedStudent); // 🟢 關鍵：發送後立刻刷新下方列表
        }
    }
        ...
        <div className="min-h-screen bg-orange-50 p-4">
            <div className="max-w-2xl mx-auto">
                ...
                {role !== 'parent' && (
                    <>
                        {/* 上半部：輸入框 */}
                        <div className="bg-white p-6 rounded-xl shadow-lg border-t-4 border-orange-500 mb-8">
                        ...
                        <div className="space-y-4">
                            <h3 className="text-xl font-bold text-gray-700 pl-2 border-l-4 border-gray-400">
                                📚 {students.find(s => s.id === selectedStudent)?.chinese_name} 的歷史紀錄
                            </h3>

                            {myBooks.length === 0 ? (
                                <div className="text-center py-6 text-gray-400">尚無紀錄</div>
                            ) : (
                                myBooks.map(book => (
                                    <div key={book.id} className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 opacity-90 hover:opacity-100 transition">
                                        <div className="flex justify-between items-center mb-2 border-b pb-2">
                                            <span className="font-bold text-orange-800">
                                                {new Date(book.created_at).toLocaleString('zh-TW')}
                                            </span>
                                            <span className="text-xs bg-gray-100 px-2 py-1 rounded text-gray-500">
                                                經手人: {book.teacher_name || '老師'}
                                            </span>
                                        </div>
                                        {book.homework && <div className="text-sm text-gray-600 mb-1">📘 <strong>作業:</strong> {book.homework}</div>}
                                        {book.content && <div className="text-sm text-gray-800">💬 <strong>評語:</strong> {book.content}</div>}
                                    </div>
                                ))
                            )}
                        </div>
